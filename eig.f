...
c
      ...
      subroutine cg_lr ( nm, n, ar, ai, wr, wi, matz, zr, zi, fv1, 
     &  iv1, ierr )

c*********************************************************************72
c
cc CG_LR gets eigenvalues and eigenvectors of a complex general matrix.
c
c  Discussion:
c
c    CG_LR calls EISPACK routines to find the eigenvalues and eigenvectors
c    of a complex general matrix using elementary transformations.
c
c  Modified:
c
c    06 March 2018
...
c           completion code described in the documentation for comlr
c           and comlr2.  the normal completion code is zero.
c
c        fv1, and iv1  are temporary storage arrays.
c
      implicit none
      ...
      call comhes ( nm, n, is1, is2, ar, ai, iv1 )

      if ( matz .eq. 0 ) then

        call comlr ( nm, n, is1, is2, ar, ai, wr, wi, ierr )

      else

        call comlr2 ( nm, n, is1, is2, iv1, ar, ai, wr, wi, zr, zi, 
     &    ierr )
      ...
      subroutine cg_qr(nm,n,ar,ai,wr,wi,matz,zr,zi,fv1,fv2,fv3,ierr)

c*********************************************************************72
c
cc CG_QR gets eigenvalues and eigenvectors of a complex general matrix.
c
...
c
   ...
   50 return
      ...
      subroutine ch3(nm,n,a,w,matz,zr,zi,fv1,fv2,fm1,ierr)

c*********************************************************************72
c
cc CH3 gets eigenvalues and eigenvectors of a complex Hermitian matrix.
c
c     this subroutine calls the recommended sequence of
c     routines from the eigensystem subroutine package (eispack)
c     to find the eigenvalues and eigenvectors (if desired)
...
c
!    Input/output, double precision A(N,N).  On input, the lower triangle of 
...
c        fv1, fv2, and  fm1  are temporary storage arrays.
c
c     Questions and comments should be directed to John Burkardt.
c
c     This version dated 05 February 2018.
c
      integer i,j,n,nm,ierr,matz
      double precision a(nm,n),w(n),zr(nm,n),zi(nm,n),
     &       fv1(n),fv2(n),fm1(2,n)
   ...
   10 call  htrid3(nm,n,a,w,fv1,fv2,fm1)
      if (matz .ne. 0) go to 20
...
c     .......... find both eigenvalues and eigenvectors ..........
   ...
   40 continue
      ...
      call tql2(nm,n,w,fv1,zr,ierr)
      if (ierr .ne. 0) go to 50
      call htrib3(nm,n,a,fm1,n,zr,zi)
   50 return
      end
...
c     this version dated october 1989.
c
      integer i,j,k,l,m,n,en,ii,ll,nm,nn,igh,ip1,
...
c     do  840 i = 1, N
c
c  CHANGED BY JVB, 03 February 2018
c
      do 840 i = 1, n - 1
         if (i .ge. low .and. i .le. igh) go to 840
c
...
c     .......... set error -- b is not positive definite ..........
 ...
 1001 return
      ...
      subroutine rg_elm(nm,n,a,wr,wi,matz,z,iv1,fv1,ierr)

c*********************************************************************72
c
cc RG_ELM computes eigenvalues and eigenvectors of a real general matrix.
c
c     this subroutine calls the recommended sequence of
c     routines from the eigensystem subroutine package (eispack)
c     to find the eigenvalues and eigenvectors (if desired)
...
c     .......... find both eigenvalues and eigenvectors ..........
   ...
   50 return
      ...
      subroutine rg_ort(nm,n,a,wr,wi,matz,z,ort,fv1,ierr)

c*********************************************************************72
c
cc RG_ORT computes eigenvalues and eigenvectors of a real general matrix.
c
c     this subroutine calls the recommended sequence of
c     routines from the eigensystem subroutine package (eispack)
c     to find the eigenvalues and eigenvectors (if desired)
...
c     .......... find eigenvalues only ..........
      ...
      go to 50
c     .......... find both eigenvalues and eigenvectors ..........
   20 call ortran(nm,n,is1,is2,a,ort,z)
      call  hqr2(nm,n,is1,is2,a,wr,wi,z,ierr)
      if (ierr .ne. 0) go to 50
c
c  Calling ORTBAK is producing nonsensical results.
c
c     call ortbak ( nm, is1, is2, a, ort, n, z )
      call  balbak(nm,n,is1,is2,fv1,n,z)
   50 return
      end
...
c          in ascending order for each asubmatrix.  if a vector error
